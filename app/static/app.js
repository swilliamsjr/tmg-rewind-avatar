...
async function fetchServerConfig() {
    try {
        ...
        // Checkboxes
        if (config.useNS === 'true') document.getElementById('useNS').checked = true;
        if (config.useEC === 'true') document.getElementById('useEC').checked = true;
        if (config.removeFillerWords === 'true') document.getElementById('removeFillerWords').checked = true;
        if (config.enableProactive === 'true') document.getElementById('enableProactive').checked = true;
        if (config.avatarEnabled === 'false') document.getElementById('avatarEnabled').checked = false;
        if (config.isPhotoAvatar === 'true') document.getElementById('isPhotoAvatar').checked = true;
        if (config.isCustomAvatar === 'true') document.getElementById('isCustomAvatar').checked = true;

        // Selects
        if (config.turnDetectionType) document.getElementById('turnDetectionType').value = config.turnDetectionType;
        if (config.voiceType) document.getElementById('voiceType').value = config.voiceType;
        if (config.avatarOutputMode) document.getElementById('avatarOutputMode').value = config.avatarOutputMode;
        if (config.avatarName) document.getElementById('avatarName').value = config.avatarName;

        // Text / textarea
        if (config.instructions) document.getElementById('instructions').value = config.instructions;
        if (config.avatarBackgroundImageUrl) document.getElementById('avatarBackgroundImageUrl').value = config.avatarBackgroundImageUrl;
        ...
        if (config.voiceTemperature) {
            document.getElementById('voiceTemperature').value = config.voiceTemperature;
            document.getElementById('voiceTempValue').textContent = config.voiceTemperature;
        }
        if (config.voiceSpeed) {
            document.getElementById('voiceSpeed').value = config.voiceSpeed;
            document.getElementById('voiceSpeedValue').textContent = config.voiceSpeed + '%';
        }

        // Refresh conditional field visibility after applying config
        updateConditionalFields();
    } catch (e) {
...
function updateConditionalFields() {
    ...
    // Subscription key vs Entra token (agents = entra, model = managed identity)
    show('subscriptionKeyField', false);
